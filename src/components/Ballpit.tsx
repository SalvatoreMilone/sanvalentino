...
/** Cuore 3D (estruso), punta sotto tonda, rosa da XConfig.colors */
function createHeartGeometry() {
  ...
  for (let t = 0; t <= 2 * Math.PI; t += 0.05) {
    const x = 16 * Math.pow(Math.sin(t), power);
    const y = 13 * Math.cos(t) - 5 * Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t);
    points.push(new Vector2(x * scale, y * scale));
  }
...
class Z extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<typeof XConfig> = {}) {
    ...
    this.spinSpeed = 0.35;
    this.rotationZ = new Float32Array(config.count);
    for (let i = 0; i < config.count; i++) {
      this.rotationZ[i] = MathUtils.randFloatSpread(Math.PI * 2);
    }
    this.#setupLights();
  ...
  update(deltaInfo: { delta: number }) {
    ...
    const d = deltaInfo.delta * this.spinSpeed;
    for (let idx = 0; idx < this.count; idx++) {
      ...
      this.rotationZ[idx] += d;
      U.rotation.set(0, 0, this.rotationZ[idx]);
      U.updateMatrix();
